...
static struct Process *mainprocess = NULL;
...
//static sAHISoundServer *_pThreadAHI = NULL;
//static int m_ahi_error=0;


static int soundMixerIsOn()
{
    return (_pThreadAHI!=NULL);
}

static void AHIS_Delete()
{
...
{
   int usePaula=0;
    if(device && strcmp(device,"ahi.device"))
